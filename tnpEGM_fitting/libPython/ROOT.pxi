# Include file inspired from root_numpy/src/setup.pxi
...
cdef extern from "TTree.h":
    cdef cppclass TTree:
        ...
        int Scan()
        void Delete(void*)
        long long SetEntries(long long)
        double GetWeight()
        int Write()
...
cdef extern from "TChain.h":
    cdef cppclass TChain(TTree):
        TChain()
        TChain(const_char*)
        int Add(const_char*, long long)
        void Print()
        int GetNtrees()
        int Add(const_char* name)
        int AddFriend(const_char* chainname, const_char* dummy)
        void Delete()
...
cdef extern from "TH1.h":
    ...
    cdef cppclass TH1D:
        TH1D(const_char* name, const_char* title, int nbinsx, double xlow, double xup)
        int Fill(double x)
        int Fill(double x, double w)
        double GetBinContent(int bin)
        void Sumw2()
        int Write(const_char* name)
        const_char* GetName()
        double IntegralAndError(int binx1, int binx2, double &err)
        TAxis* GetXaxis()
        int GetNbinsX()
